clear
capture program drop icc_twoGroup_comparison
program define icc_twoGroup_comparison,rclass
version 19.0
 syntax [, n(integer 1000) prev(real 0.9) target_icc(real 0.01) trtEff(real .9)  m(integer 150)  ] 
qui{
	drop _all
	set obs 2
	gen intv = _n-1
	...
	* Create data set
	...
	// Between-cluster variance, ctrl group
	local sig2_u = ( `target_icc'* (`prev'*(1-`prev') ) )/(1-`target_icc') 
	local sig_u = sqrt(`sig2_u')
	* Precision parameter for ctrl group
	local alpha = (`prev'*(1-`prev'))/(`sig2_u') - 1
	// Between-cluster variance, trtmt group
	local sig2_u_trt = ( `target_icc'* ((`prev'*`trtEff')*(1-`prev'*`trtEff') ) )/(1-`target_icc') 
	local sig_u_trt = sqrt(`sig2_u')
	* Precision parameter for trt group
	local alpha_trt = ((`prev'*`trtEff')*(1-(`prev'*`trtEff')))/(`sig2_u_trt') - 1
	gen p = rbeta(`alpha'*`prev',`alpha'*(1-`prev')) if intv==0 //`prev' if intv==0
	replace p = rbeta(`alpha_trt'*(`prev'*`trtEff'),`alpha_trt'*(1-(`prev'*`trtEff'))) if intv==1 // `prev'* `trtEff' if intv==1
	gen clusP = p  
	expand clusSize

	* Generate outcome
	gen outcome = rbinomial(1, clusP)
	mixed outcome i.intv || clusID:, reml
	estat icc
...
******** Scenario 1
simulate, seed(123) reps(1000): icc_twoGroup_comparison, n(100) ///
...
******** Scenario 6
simulate, seed(123) reps(1000): icc_twoGroup_comparison, n(50) ///
							prev(0.5) target_icc(0.1) m(50) 				
gen lb = identityICC - 1.96* identityICC_se
gen ub = identityICC + 1.96* identityICC_se
gen withinCI = 0
...
******** Scenario 11
simulate, seed(123) reps(1000): icc_twoGroup_comparison, n(10) ///
							prev(0.5) target_icc(0.1) m(50) 				
gen lb = identityICC - 1.96* identityICC_se
gen ub = identityICC + 1.96* identityICC_se
gen withinCI = 0
...
save "ICC results_intv.dta", replace